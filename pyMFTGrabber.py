...
def decodeATRHeader(s):
    ...
    if d['type'] == 0xffffffff:
        return d
    d['len'] = struct.unpack("<L", bytes(s[4:8]))[0]
    d['res'] = struct.unpack("B", bytes(s[8:9]))[0]
    d['nlen'] = struct.unpack("B", bytes(s[9:10]))[0]  # This name is the name of the ADS, I think.
    d['name_off'] = struct.unpack("<H", bytes(s[10:12]))[0]
    d['flags'] = struct.unpack("<H", bytes(s[12:14]))[0]
    d['id'] = struct.unpack("<H", bytes(s[14:16]))[0]
    if d['res'] == 0:
        d['ssize'] = struct.unpack("<L", bytes(s[16:20]))[0]
        d['soff'] = struct.unpack("<H", bytes(s[20:22]))[0]
        d['idxflag'] = struct.unpack("<H", bytes(s[22:24]))[0]
    else:
        d['start_vcn'] = struct.unpack("<d", bytes(s[16:24]))[0]
        d['last_vcn'] = struct.unpack("<d", bytes(s[24:32]))[0]
        d['run_off'] = struct.unpack("<H", bytes(s[32:34]))[0]
        d['compusize'] = struct.unpack("<H", bytes(s[34:36]))[0]
        d['f1'] = struct.unpack("<I", bytes(s[36:40]))[0]
        d['alen'] = struct.unpack("<d", bytes(s[40:48]))[0]
        d['ssize'] = struct.unpack("<d", bytes(s[48:56]))[0]
        d['initsize'] = struct.unpack("<d", bytes(s[56:64]))[0]
...
if __name__ == '__main__':
    ...
    ntfsdrive = open(r'%s' % options.input, 'rb')
    if os.name == 'nt':
